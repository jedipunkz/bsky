...
func (m *Model) renderSearchResults(height int) string {
	...
	for i := start; i < end; i++ {
		...
		if selected {
			...
			rendered = selectedPostStyle.Render(content)
		} else {
...
func (m *Model) renderProfilePosts(width, height int) string {
	...
	for i := start; i < end; i++ {
		...
		if selected {
			...
			rendered = selectedPostStyle.Render(postContent)
		} else {
...
// element and pads every line to innerWidth with background-colored spaces.
// The caller must NOT pass this content to a style with Width() set, to avoid
// re-measurement that would cause extra blank lines from wrapping.
